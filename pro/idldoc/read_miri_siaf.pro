function read_MIRI_siaf,redo=redo,verbose=verbose
	if n_elements(verbose) ne 1 then verbose=1
	if n_elements(redo) ne 1 then redo=1
	rcs_id="$Id: read_miri_siaf.pro,v 1.2 2018/02/27 20:43:40 penton Exp $"
	...
	if redo then begin
		...
		file='MIRI_SIAF.txt'
		apercsvfile='MIRI_SIAF_APERTURES.csv'
		scidlcsvfile='MIRI_SIAF_SCI_IDL.csv'
		savefile='MIRI_SIAF.dat'
		...
		siaf_MIRI_aper = READ_CSV(siafdir+apercsvfile, HEADER=ACsvHeader, N_TABLE_HEADER=1, TABLE_HEADER=CsvTableHeader,TYPES=TYPES_APER)
		siaf_MIRI_sci_idl=READ_CSV(siafdir+scidlcsvfile, HEADER=SCsvHeader, N_TABLE_HEADER=1, TABLE_HEADER=CsvTableHeader,TYPES=TYPES_SCI_IDL)
		; rw 333 @ NOON
		;
		...
		for i=0,na-1 do begin
			str=ACsvHeader[i]+'=siaf_MIRI_aper.FIELD'+string1i(i+1,format='(I02)') ; +'"'
			st=execute(str)
			;print,i,str,ST
		endfor
		na=n_elements(SCsvHeader)
		for i=0,na-1 do begin
			str=SCsvHeader[i]+'=siaf_MIRI_sci_idl.FIELD'+string1i(i+1,format='(I02)') ; +'"'
			st=execute(str)
			;print,i,str,ST
		endfor
		...
		help,st,/str
		MIRI_siaf_str=st
		save,file=datdir+savefile,MIRI_siaf_str,/compress,verbose=verbose
		heap_gc
	endif else begin
		restore,datdir+savefile,verbose=verbose
	endelse
	return,MIRI_siaf_str
end
