function jwst_get_siaf_allsi,redo=redo,verbose=verbose,debug=debug
	if n_elements(debug) ne 1 then debug=0
	if n_elements(verbose) ne 1 then verbose=0
	if n_elements(redo) ne 1 then redo=1
	;
	fgs=jwst_read_si_siaf('FGS',redo=redo,verbose=verbose,debug=debug)
	niriss=jwst_read_si_siaf('NIRISS',redo=redo,verbose=verbose,debug=debug)
	miri=jwst_read_si_siaf('MIRI',redo=redo,verbose=verbose,debug=debug)
	nircam=jwst_read_si_siaf('NIRCAM',redo=redo,verbose=verbose,debug=debug)
	nirspec=jwst_read_si_siaf('NIRSPEC',redo=redo,verbose=verbose,debug=debug)

	siaf={fgs:fgs,miri:miri,nircam:nircam,nirspec:nirspec,niriss:niriss}
	return,siaf
end
function jwst_read_si_siaf,SI,redo=redo,verbose=verbose,debug=debug
	if n_elements(debug) ne 1 then debug=0
	if n_elements(verbose) ne 1 then verbose=0
	...
	savefile=uSI+'_SIAF.dat'
	if redo then begin
		if (verbose ge 1) then message,/info,'RE-CREATING the JWST SIAF for '+uSI+' and CREATING SAVEFILE as '+datdir+savefile
		...
		if (verbose gt 1) then message,/info,'RESTORING the JWST SIAF for '+uSI+' from '+datdir+savefile
