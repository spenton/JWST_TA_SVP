function read_FGS_siaf,redo=redo,verbose=verbose
	if n_elements(verbose) ne 1 then verbose=0
	if n_elements(redo) ne 1 then redo=1
	rcs_id="$Id: read_fgs_siaf.pro,v 1.3 2018/03/15 12:12:32 penton Exp $"
	...
	if redo then begin
		...
		file='FGS_SIAF.txt'
		apercsvfile='FGS_SIAF_APERTURES.csv'
		scidlcsvfile='FGS_SIAF_SCI_IDL.csv'
		savefile='FGS_SIAF.dat'
		...
		if verbose then message,/info,'READING '+siafdir+apercsvfile
		siaf_FGS_aper = READ_CSV(siafdir+apercsvfile, HEADER=ACsvHeader, N_TABLE_HEADER=1, TABLE_HEADER=CsvTableHeader,TYPES=TYPES_APER)
		siaf_FGS_sci_idl=READ_CSV(siafdir+scidlcsvfile, HEADER=SCsvHeader, N_TABLE_HEADER=1, TABLE_HEADER=CsvTableHeader,TYPES=TYPES_SCI_IDL)
		; rw 333 @ NOON
		;
		...
		for i=0,na-1 do begin
			str=ACsvHeader[i]+'=siaf_FGS_aper.FIELD'+string1i(i+1,format='(I02)') ; +'"'
			st=execute(str)
			;print,i,str,ST
		endfor
		na=n_elements(SCsvHeader)
		for i=0,na-1 do begin
			str=SCsvHeader[i]+'=siaf_FGS_sci_idl.FIELD'+string1i(i+1,format='(I02)') ; +'"'
			st=execute(str)
			;print,i,str,ST
		endfor
		...
		FGS_siaf_str=st
		save,file=datdir+savefile,FGS_siaf_str,/compress,verbose=verbose
		heap_gc
	endif else begin
