...
;
; Usage:
;          v23GWA=JWST_DET_TO_GWA(xyDET)
;
; Inputs:
;	xyDET: in, required, type=structure
;     xDETprime: contains the DET out side X coordinate(s)
;     yDETprime: contains the DET out side Y coordinate(s)
;
;	v23GWA: out, structure
;		contains the output structure
;     v2: contains the GWA V2 coordinate(s) (arcseconds)
;     v3: contains the GWA V3 coordinate(s) (arcseconds)
...
pro test_det_to_sky,apername,verbose=verbose,debug=debug
	...
	; Step 3) GWAout (prime) to SKY
	;
	testdir=jwst_get_nrs_testdir
	testfile=''
	readcol,testdir+testfile,v1,v2 ; ...
	gwain=det_to_gwa(testdata)
	gwaqut=gwain_to_gwaout(gwain)
	sky=gwaout_to_sky(gwaout)
end
pro test_det_to_gwa,apername,verbose=verbose,debug=debug
	if n_elements(debug) ne 1 then debug=0
	if n_elements(verbose) ne 1 then verbose=0
	; Read in the NIRSPEC TEAM TEST FILES and test
	;
...
pro test_det_to_gwain,apername,verbose=verbose,debug=debug,detnum=detnum
	...
	for t=0,nt -1 do begin
		print,'---'
		xy=testxy[*,t]
		v23GWAin=jwst_nrs_det_to_gwain(xyDET,detnum,aper1,debug=debug,verbose=verbose)
		print,'DET',xyDET
		print,'GWAin',v23GWA
		print,'---'
	endfor
...
function jwst_nrs_gwaout_to_gwain,xyGWAout
		xyGWAin=xyGWAout
		return,xyGWAin
end
function jwst_nrs_DET_in_to_out,xyDETin
		;
...
function jwst_nrs_DET_to_GWA,xyDETprime,apername,verbose=verbose,debug=debug,$
