...
pro test_si_sti,si,apername,verbose=verbose,debug=debug
	...
	testxy=[[1,1],[1024.5,1024.5],[1000,2000],[2000,1000]]
	ns=size(testxy,/dimensions)
	nt=ns[1]
	aper1='NRS1_FULL_OSS'
	aper2='NRS2_FULL_OSS'
	for t=0,nt -1 do begin
		...
		xy=testxy[*,t]
		help,xy
		ns1=jwst_si_sci_to_idl(xy,'NIRSPEC',aper1,debug=debug,verbose=verbose)
		ns2=jwst_si_sci_to_idl(xy,'NIRSPEC',aper2,debug=debug,verbose=verbose)
		print,'NRS1',ns1.xyidl
		print,'NRS2',ns2.xyidl
		print,'---'
	endfor
	stop
end
function jwst_si_sci_to_idl,xySci,si,apername,verbose=verbose,debug=debug,$
	...
	if n_elements(debug) ne 1 then debug=0
	if n_elements(verbose) ne 1 then verbose=0
	rcs_id='$Id: jwst_si_sci_to_idl.pro,v 1.2 2018/04/10 14:23:57 penton Exp penton $'
	FF='(F12.6)'
