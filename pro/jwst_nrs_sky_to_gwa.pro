...
;
; Usage:
;          GWAout=JWST_SKY_TO_GWAout(v23SKY)
;
; Inputs:
;	v23SKY: in, required, type=structure
...
pro test_sky_to_gwaout,apername,verbose=verbose,debug=debug,detnum=detnum
	...
	for t=0,nt -1 do begin
		print,'---'
		xy=testxy[*,t]
		v23SKY=jwst_nrs_sky_to_gwaout(xyDET,detnum,aper1,debug=debug,verbose=verbose)
		print,'DET',xyDET
		print,'SKY',v23SKY
		print,'---'
	endfor
...
function jwst_nrs_gwa_in_to_out,xyGWAin
		...
		; not-prime = in
		;
		rcs_id="$Id: jwst_nrs_sky_to_gwa.pro,v 1.1 2018/04/10 14:23:57 penton Exp penton $"
		;
		; section 5.5.2
		;
		xgwa=xyGWA.xgwa
...
function jwst_nrs_SKY_TO_GWAout,xyGWAprime,apername,verbose=verbose,debug=debug,$
